@extends('welcome')
@section('title','SELLS-ERP:Client')
@push('css')
...
<tr>
  ...
  <td>{{$cl_v->email}}</td>
  <td>{{$cl_v->phone}}</td>
  <td><img src="{{asset($cl_v->image)}}" alt="" style="width: 90px"></td>
  <td>{{$cl_v->address}}</td>


  <td>
    <a href="" class="btn btn-info edit-client"  data-toggle="modal" data-target="#modal-editclient" client_id="{{$cl_v->id}}">Edit</a>
        <button class="btn btn-danger" onclick="deleteTag(<?php echo $cl_v->id ?>)"><i class="material-icons">delete</i></button>
        <form id="delete-form-{{$cl_v->id}}" action="{{route('admin.client.delete',$cl_v->id)}}" method="post" style="display: none"> {{csrf_field()}}</form>
  </td>
</tr>
        ...
        <div class="modal fade" id="modal-editclient">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <form role="form" action="{{route('admin.client.update')}}" method="post" enctype="multipart/form-data">
                {{@csrf_field()}}
                <!-- text input -->
                <div class="form-group">
                  <label>Name</label>
                  <input type="hidden" name="id" id="id" class="form-control" placeholder="Enter Client name" required>
                  <input type="text" name="name" id="name" class="form-control" placeholder="Enter Client name" required>
                </div>
                  <div class="form-group">
                  <label>Email</label>
                  <input type="text" name="email" id="email" class="form-control" placeholder="Enter Client Email" required>
                </div>
                  <div class="form-group">
                  <label>Phone</label>
                  <input type="text" name="phone" id="phone" class="form-control" placeholder="Enter Client Phone" required>
                </div>
                <!-- textarea -->
                <div class="form-group">
                  <label>Address</label>
