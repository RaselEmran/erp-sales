...
<form action=""  name="frmUser" method="post">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example1">
		<div class="alert alert-success">
			<h2 style="text-align:center; font-family:cursive;">Backup Or Destroy Supplier</h2>
		</div>
		<thead>
<button type="button" class="btn btn-info" onClick="setUpdateAction();"><i class="fa fa-fw fa-undo" style="margin-right: 5px"></i></button><button type="button" class="btn btn-danger" onClick="setDeleteAction();"><i class="fa fa-fw fa-trash"></i></button>
		...
		<tbody>
       @foreach ($supplier as $key=> $element)
			...
			<tr>
				...
				<td style="text-align:center; font-family:cursive; font-size:18px;">{{$element->email}}</td>
        <td style="text-align:center; font-family:cursive; font-size:18px;">{{$element->phone}}</td>
        <td style="text-align:center; font-family:cursive; font-size:18px;">{{$element->address}}</td>
...
document.frmUser.action = "{{ route('admin.backup.resetsupplier') }}";
...
Swal.fire({
  ...
  if (result.value) {
  	...
  	document.frmUser.action = "{{ route('admin.backup.destroysupplier') }}";
    document.frmUser.submit();
    Swal.fire(
      'Deleted!',
